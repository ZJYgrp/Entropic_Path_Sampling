...
mpirun -bynode -cpus-per-proc $OMP_NUM_THREADS -np $MPI_NUM_PROCS --mca btl tcp,self,vader --report-bindings ../exec/PARENT.x ${NAME}.bat ${NAME}.par ${BBINS1D} ${ABINS1D} ${DBINS1D} ${BBINS2D} ${ABINS2D} ${DBINS2D} >> log.txt 2>&1
...
# Depending on the architecture of your cluster/workstation as well as your Open MPI version, you might want to change the mpirun parameters. Especially if you are using InfiniBand you might want to change "tcp" to "openib"
mpirun -bynode -cpus-per-proc $OMP_NUM_THREADS -np $MPI_NUM_PROCS --mca btl tcp,self,vader --report-bindings ../exec/get_PAR_MIST.x ${NAME}.par ${NAME}_MIST.txt >> log.txt 2>&1
